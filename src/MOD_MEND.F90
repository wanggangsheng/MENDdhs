...
!    print*,"ok:",sINI%VARopt_int
    ...
    do i = 1, sINI%nVARopt 
        ...
        else if (trim(sINI%VARobj(j)).eq."AVGr") then
            sINI%rOBJ(i) = f1RAVG(sINI%VARopt_int(i,2),sINI%dOBS_opt(ibeg:iend,2), sINI%dSIM_opt(ibeg:iend,2), const_FillValue)
        else if (trim(sINI%VARobj(j)).eq."MAR2") then
            sINI%rOBJ(i) = fMAR2(sINI%VARopt_int(i,2), sINI%dOBS_opt(ibeg:iend,2), sINI%dSIM_opt(ibeg:iend,2), &
                            sINI%VARobj_tol(j), const_FillValue)
        else !! "MARE"
            sINI%rOBJ(i) = 0.5d0*fMARE(sINI%VARopt_int(i,2), &
                                sINI%dOBS_opt(ibeg:iend,2), sINI%dSIM_opt(ibeg:iend,2), const_FillValue) &
                         + 0.5d0*f1RAVG(sINI%VARopt_int(i,2), &
                                sINI%dOBS_opt(ibeg:iend,2), sINI%dSIM_opt(ibeg:iend,2), const_FillValue)
        end if
    end do
    ...
    if(isnan(fMEND_OBJ)) then
        write(*,'(A10,2x,50A10)')"Obj",Name_PAR
...
!!"Hour","kPOM1","kPOM2","kMOM","kDOM","kMB","phi","rMBA"
    ...
    kMBa_in = (sPAR%Vg + sPAR%Vm)/sPAR%Yg*phi + sOUT%CFLUX%MBD_to_MBA/sINP%CPOOL%MBA
...
!        write(*,*)'debug= ', sOUT%CFLUX%DOM_to_MBA, sOUT%MNFLUX%CO2_gmo
...
!            sINP % CADDI % DOMadd = sINP%CADD%DOMadd*frISOadd
            ...
            CALL sOUT_OPT_h(sINI%nVARopt,24,lp,dSIM,sOUT,sINI%VARopt_int,sPAR%VdPOM(1:2))
...
!    sPAR%VNup_MB    = sPAR%VNup_MB * phi  !!5/25/2017
    ...
    sOUT%MNFLUX%Nim_NH4 = sPAR%VNup_MB *phi* sINP%MNPOOL%NH4 &
                        * sINP%NPOOL%MBA/(sPAR%KsNH4_MB*(rtp1 + sINP%NPOOL%MBA/sPAR%KsNH4_MB))
    sOUT%MNFLUX%Nim_NO3 = sPAR%VNup_MB *phi* sINP%MNPOOL%NO3 &
...
!    sPAR%iKinetics = sINI%iKinetics
    ...
    !!sPARbase
    sPARbase%fINP       = 1.0!!xx(3)
    sPARbase%VdPOM(1)   = xx(3)!!xx(4)
    sPARbase%VdPOM(2)   = xx(4) !!xx(4)
    sPARbase%VdMOM      = xx(5) !!xx(4) !!xx(5)
    sPARbase%KsPOM(1)   = xx(6)
    sPARbase%KsPOM(2)   = xx(7) !!xx(6)*0.1  !!xx(7)
    sPARbase%KsMOM      = xx(8) !!xx(6)*10.0 !!xx(8)
    sPARbase%Qmax       = xx(9)
    sPARbase%Kba        = xx(10)
    sPARbase%Kdes       = xx(11)
...
!    sPARbase%Ygsl       = xx(21)  !!no Ygsl in sPAR
    ...
    sPARbase%wdie       = xx(22)  !! 8/16/2016, NO use anymore
    ...
    !! Cellulase
!    wp_scalar_opt = fSWP(wp, BIOME, SOM)
...
!    CUE_slope = -1.D0*xx(21)
    ...
    sPAR%wdie = sPARbase%wdie  !! no use any more
...
!    if(sINI%iSWP_die.eq.0) then
    ...
    !! test    
!        wp_scalar_low = 1.d0 - fSWP(wp, BIOME, SOM)
!    else
...
!            case (6)    !!DOM_SIO
!                dSIM(iHour,j) = sOUT%CPOOLI(2)%DOM
            case (7)    !!SOM
...
!                dSIM(iHour,j) = sOUT%CPOOLI(2)%SOM
            case (9)    !!ENZ_LIG, ENZ Activity = ENZ concentration*specific enzyme activity
                dSIM(iHour,j) = sOUT%CPOOL%ENZP(1)*vENZ(1)  !!sPAR%VdPOM(1)
            case (10)   !!ENZ_CEL, ENZ Activity
                dSIM(iHour,j) = sOUT%CPOOL%ENZP(2)*vENZ(2)  !!sPAR%VdPOM(2)
            case (11)    !!SOM_CN
