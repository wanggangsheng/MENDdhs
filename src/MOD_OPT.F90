...
!c  PRINT THE RESULTS FOR THE INITIAL POPULATION
      ...
      write(sPAR_SCE%iFout1,format610) sPAR_SCE%parName 
     ...
     &               (bestx(j),j=1,nPar)
      ...
      if (iprint .eq. 1) then
        ...
        do i = 1, npt1
          write(sPAR_SCE%iFout1,format660) xf(i),x(i,1:nPar)!!(x(i,j),j=1,nPar)
        end do
...
!c  PRINT THE FINAL PARAMETER ESTIMATE AND ITS FUNCTION VALUE
      ...
      write(sPAR_SCE%iFout3,format520) bestf,bestx  
